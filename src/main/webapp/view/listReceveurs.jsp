...
<table class="min-w-full bg-white rounded-xl shadow-lg overflow-hidden">
    ...
    <c:forEach var="r" items="${receveurs}">
        <tr class="even:bg-blue-100 odd:bg-blue-50 hover:bg-blue-200">
            ...
            <td class="border px-4 py-2 text-center font-semibold">${r.besoinPoches}</td>
            <td class="border px-4 py-2 text-center">
                <button onclick="openModal('${r.id}', '${r.nom}', '${r.prenom}', '${r.cin}', '${r.telephone}', '${r.dateNaissance}', '${r.sexe}', '${r.groupeSanguin}', '${r.besoinPoches}')"
...
<div id="editModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden">
    <div class="bg-white p-6 rounded-lg shadow-lg w-1/2">
        <h2 class="text-2xl font-bold mb-4 text-blue-600 text-center">Modifier Receveur</h2>
        <form method="post" class="space-y-3">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-gray-700 mb-1">Situation :</label>
                    <select name="situation" id="editSituation" class="w-full border rounded-lg p-2">
...
<script>
    ...
    function openModal(id, nom, prenom, cin, tel, date, sexe, groupe, besoin) {
        ...
        if (besoin >= 4) situation = "CRITIQUE";
        else if (besoin === "3" || besoin === 3) situation = "URGENT";
